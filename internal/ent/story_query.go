...
func (sq *StoryQuery) QuerySentences() *SentenceQuery {
	query := (&SentenceClient{config: sq.config}).Query()
...
func (sq *StoryQuery) Clone() *StoryQuery {
	...
	return &StoryQuery{
		...
		withSentences: sq.withSentences.Clone(),
...
// WithSentences tells the query-builder to eager-load the nodes that are connected to
// the "sentences" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *StoryQuery) WithSentences(opts ...func(*SentenceQuery)) *StoryQuery {
	query := (&SentenceClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withSentences = query
	return sq
}
...
func (sq *StoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Story, error) {
	...
	if query := sq.withSentences; query != nil {
		if err := sq.loadSentences(ctx, query, nodes,
			func(n *Story) { n.Edges.Sentences = []*Sentence{} },
			func(n *Story, e *Sentence) { n.Edges.Sentences = append(n.Edges.Sentences, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StoryQuery) loadSentences(ctx context.Context, query *SentenceQuery, nodes []*Story, init func(*Story), assign func(*Story, *Sentence)) error {
	...
	query.withFKs = true
	query.Where(predicate.Sentence(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(story.SentencesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.story_sentences
		if fk == nil {
			return fmt.Errorf(`foreign-key "story_sentences" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "story_sentences" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (sq *StoryQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(story.Table, story.Columns, sqlgraph.NewFieldSpec(story.FieldID, field.TypeUUID))
	_spec.From = sq.sql
